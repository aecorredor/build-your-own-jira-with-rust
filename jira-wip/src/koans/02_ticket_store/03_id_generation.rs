mod id_generation {
    ...
    /// It's a lightweight technique to add a semantic layer to the underlying
    /// data type.
    ///
    ...
    /// ```
    /// If we want to be sure we aren't mixing up ticket ids and `u32` variables
    /// with a different semantic meaning, we would have to create a new type,
    /// e.g. `struct TicketId(u32)`.
    /// For now this doesn't feel necessary - we don't have many `u32`s flying
    /// around.
    ...
    impl TicketStore {
        ...
        /// So far we have taken the `id` as one the parameters of our `save`
        /// method.
        ///
        /// What happens when you call save passing two different tickets with
        /// the same id?
        /// We have enforced with a test our expectation: the second ticket
        /// overwrites the first.
        ///
        /// The other option would have been to error out.
        ///
        /// This isn't how JIRA works: you don't get to choose the id of your
        /// ticket, it's generated for you and its uniqueness is guaranteed.
        /// There is also another peculiarity: ids are integers and they are
        /// monotonically increasing (the first ticket on a board will be
        /// `BOARDNAME-1`, the second `BOARDNAME-2` and so on).
        ///
        /// We want the same behaviour in our clone, IronJira.
        /// `TicketStore` will take care of generating an id for our ticket and
        /// the id will be returned by `save` after insertion.
